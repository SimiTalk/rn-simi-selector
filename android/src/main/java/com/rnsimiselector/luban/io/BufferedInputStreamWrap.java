...
import com.rnsimiselector.luban.io.ArrayPoolProvide;
...
public class BufferedInputStreamWrap extends FilterInputStream {
    ...
    BufferedInputStreamWrap(InputStream in, int bufferSize) {
        super(in);
        buf = com.rnsimiselector.luban.io.ArrayPoolProvide.getInstance().get(bufferSize);
    }
    ...
    public synchronized void release() {
        if (buf != null) {
            com.rnsimiselector.luban.io.ArrayPoolProvide.getInstance().put(buf);
            buf = null;
        }
    }
    ...
    private int fillbuf(InputStream localIn, byte[] localBuf) throws IOException {
        ...
        if (markPos == 0 && markLimit > localBuf.length && count == localBuf.length) {
            ...
            byte[] newbuf = com.rnsimiselector.luban.io.ArrayPoolProvide.getInstance().get(newLength);
